...
util.AddNetworkString("gms_MakeProcessBar")
util.AddNetworkString("gms_ResetPlayer")
util.AddNetworkString("gms_StopProcessBar")
util.AddNetworkString("gms_MakeLoadingBar")
util.AddNetworkString("gms_StopLoadingBar")
util.AddNetworkString("gms_MakeSavingBar")
util.AddNetworkString("gms_StopSavingBar")
util.AddNetworkString("gms_SetSkill")
util.AddNetworkString("gms_SetXP")
util.AddNetworkString("gms_SetResource")
util.AddNetworkString("gms_SetMaxResources")
util.AddNetworkString("gms_OpenCombiMenu")
util.AddNetworkString("gms_sendmessage")
util.AddNetworkString("gms_CreateFadingProp")
util.AddNetworkString("gms_sendachievement")
util.AddNetworkString("gms_setneeds")
...
function PlayerMeta:OpenCombiMenu(str)
	net.Start("gms_OpenCombiMenu")
		net.WriteString(str)
	net.Send(self)
end

function PlayerMeta:SendAchievement(text)
	net.Start("gms_sendachievement")
		net.WriteString(text)
	net.Send(self)
...
function PlayerMeta:SetXP(skill, int)
	...
	net.Start("gms_SetXP")
		net.WriteString(skill)
		net.WriteInt(self:GetXP(skill), 32)
	net.Send(self)
end
...
function PlayerMeta:UpdateNeeds()
	net.Start("gms_setneeds")
		net.WriteInt(self.Sleepiness, 32)
		net.WriteInt(self.Hunger, 32)
		net.WriteInt(self.Thirst, 32)
		net.WriteInt(self.Oxygen, 32)
...
function PlayerMeta:MakeLoadingBar(msg)
	net.Start("gms_MakeLoadingBar")
		net.WriteString(msg)
	net.Send(self)
end

function PlayerMeta:StopLoadingBar()
	net.Start("gms_StopLoadingBar")
	net.Send(self)
end

function PlayerMeta:MakeSavingBar(msg)
	net.Start("gms_MakeSavingBar")
		net.WriteString(msg)
	net.Send(self)
end

function PlayerMeta:StopSavingBar()
	net.Start("gms_StopSavingBar")
	net.Send(self)
end
...
function EntityMeta:SetResourceDropInfoInstant(strType, int)
	...
	net.Start("gms_SetResourceDropInfo")
		...
		net.WriteString(string.gsub(strType, "_", " "))
		net.WriteInt(self.Amount, 32)
	net.Broadcast()
end
...
function EntityMeta:SetFoodInfo(strType)
	timer.Simple(0.5, function()
		self:SetFoodInfoInstant(strType)
	end)
end

function EntityMeta:SetFoodInfoInstant(strType)
	local strType = strType or "Error"
	net.Start("gms_SetFoodDropInfo")
		net.WriteString(self:EntIndex())
		net.WriteString(string.gsub(strType, "_", " "))
	net.Broadcast()
end
...
function GM:PlayerInitialSpawn(ply)
	...
	for _, v in ipairs(ents.FindByClass("gms_food")) do
		timer.Simple(time, function()
			net.Start("gms_SetFoodDropInfo", ply)
				net.WriteString(v:EntIndex())
				net.WriteString(string.gsub(v.Name or "ERROR", "_", " "))
			net.Send(ply)
		end)
		time = time + 0.1
	end
		...
		if (ply:Alive()) then
			...
			if (!ply:GetNWBool("AFK")) then
				...
				if (ply.Thirst > 0) then
					ply.Thirst = math.Clamp(ply.Thirst + AddThirst, 0, 1000)
				end
...
timer.Create("AlertTimer", 6, 0, function()
	...
	for k, ply in pairs(player.GetAll()) do 
		...
		if (ply.Hunger < 125) then
			ply:EmitSound(Sound(AlertSoundsHunger[math.random(1, #AlertSoundsHunger)]), 100, math.random(95, 105))
			table.insert(StatusMsgs, "starving")
		end
		
		if (ply.Thirst < 125) then
			ply:EmitSound(Sound(AlertSoundsThirst[math.random(1, #AlertSoundsThirst)]), 100, math.random(95, 105))
			table.insert(StatusMsgs, "thirsty")
		end
		
		if (ply.Sleepiness < 125) then
			ply:EmitSound(Sound(AlertSoundsSleep[math.random(1, #AlertSoundsSleep)]), 100, math.random(95, 105))
			table.insert(StatusMsgs, "tired")
		end
		
		if next(StatusMsgs) == nil then continue end
		
		if #StatusMsgs == 1 then
			ply:ChatPrint("You are " .. StatusMsgs[1] .. ".")
		elseif #StatusMsgs == 2 then
			ply:ChatPrint("You are " .. StatusMsgs[1] .. " and " .. StatusMsgs[2] .. ".")
		elseif #StatusMsgs == 3 then
			local Last = table.remove(StatusMsgs, #StatusMsgs)
			ply:ChatPrint("You are " .. table.concat(StatusMsgs, ", ") .. " and " .. Last .. "!")
		end
	end
end)
